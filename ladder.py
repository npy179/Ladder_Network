...
layer_sizes = [2195, 1000, 500, 250,  1]
...
vpred_cost = -tf.reduce_mean(tf.reduce_sum(outputs*tf.log(y), 1))  # cost used for prediction
...
dataset = SemiDataset(label_dataset, unlabel_dataset, 2)
test_dataset = Label_Dataset(label_dataset, 2)
...
print "=== Training ==="
#print "Initial Accuracy: ", sess.run(accuracy, feed_dict={inputs: test_dataset.next_batch(), outputs: test_dataset.next_batch(), training: False}), "%"
...
for i in tqdm(range(i_iter, num_iter)):
    ...
    if (i > 1) and ((i+1) % (num_iter/num_epochs) == 0):
        ...
        with open('train_log', 'ab') as train_log:
            ...
            log_i = [epoch_n] + sess.run([accuracy], feed_dict={inputs: test_dataset.next_batch() , outputs: test_dataset.next_batch(), training: False})
